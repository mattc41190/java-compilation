...
case "$1" in
        ...
        # Recall as well, that packages are inexoribly linked to directory in this case when the user 
        # invokes `java` and provides a classpath of app:$CALC the following packages are available to the
        # JRE: com.door_store.Store.class, com.wacky_math.Calculator.class (by virtue of the JAR classpath 
        # rules), and the classpath of the class with the main method which intend to invoke com.matthewcale.App
        basic_run)
        ...
        # the entry point gets placed into a file called a Manifest. The Manifest file will be covered in
